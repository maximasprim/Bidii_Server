...
class AIBranchMatch:
    ...
    Superseded as of the geocode-based redesign below by AIGeocodeResult -
    kept here (and still implemented by both providers) for backward
    compatibility since nothing outside this package should ever need to
    stop working just because branch_assignment.py changed which method it
    calls. Not used by branch_assignment.py's live code path anymore.
    """
...
@dataclass
class AIGeocodeResult:
    """
    Result of asking the AI to estimate real-world coordinates for an
    applicant's free-text location (a town, estate, road, or landmark),
    rather than asking it to directly pick among a list of branches.

    This is deliberately a smaller, easier task for the model than
    AIBranchMatch's "compare N candidates and pick one" - a model only has
    to recall where one named place roughly is, which it tends to do far
    more reliably than multi-way geographic comparison, especially on
    smaller/cheaper models. branch_assignment.py then does the actual
    nearest-branch comparison itself, deterministically, using real
    haversine distance against each active Branch's stored lat/lng - so
    the same (lat, lng) estimate always produces the same branch decision,
    and that decision is fully explainable/reproducible after the fact
    (unlike trusting a model's own pick, which can vary run to run for the
    exact same input).

    (lat, lng) is validated by the parser to fall within a generous
    Kenya bounding box before being trusted - see parse_geocode_response.
    """
...
class AIProvider(ABC):
    ...
    def geocode_location(
        self, *, location_text: str, county: str | None, model: str, timeout_seconds: int
    ) -> AIGeocodeResult: ...
