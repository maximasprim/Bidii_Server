...
# Generous bounding box around Kenya (with margin for border towns) used to
# sanity-check every AI-estimated (lat, lng) before it's trusted for a
# haversine distance calculation - see parse_geocode_response. Kenya itself
# is roughly lat -4.72 to 5.02, lng 33.9 to 41.91; padded out further here
# so a slightly-off estimate for a genuine border-adjacent town isn't
# rejected, while a wildly wrong guess (wrong country/hemisphere) is.
...
GEOCODE_SYSTEM_PROMPT = (
    "You are a Kenya geography expert helping geocode loan applicants' free-text locations for "
    "Bidii Credit, a Kenyan lending company. Given an applicant's own description of where they "
    "are (a town, estate, road, landmark, or area) and, for weak context only, which county they "
    "selected on a form, estimate the approximate real-world latitude and longitude of the place "
    "they described, using your knowledge of Kenyan geography. The county they selected may be "
    "wrong or imprecise (applicants sometimes pick the nearest option on a short list) - always "
    "prioritize the specific place name they typed over the county when the two seem to disagree. "
    "Return ONLY a JSON object matching the exact schema you're given - no prose, no markdown "
    "fences, no commentary outside the JSON."
)

GEOCODE_JSON_SCHEMA_HINT = """
Return exactly this JSON shape:
{
  "lat": <number, estimated latitude in decimal degrees>,
  "lng": <number, estimated longitude in decimal degrees>,
  "reasoning": "<one short sentence naming the specific place you geocoded and why>"
}
Always give your single best estimate, even if the description is vague (e.g. only an estate or
landmark name, no town) - approximate is fine, never refuse and never leave a field out.
"""


def build_geocode_prompt(location_text: str, county: str | None) -> str:
    county_line = f"County selected on the form (weak context, may be imprecise): {county}" if county else ""
    return f"""APPLICANT'S STATED LOCATION
{location_text}
{county_line}
...
def parse_geocode_response(raw: str, *, provider: str, model: str) -> AIGeocodeResult:
    try:
        data = json.loads(_strip_json_fences(raw))
    except (json.JSONDecodeError, TypeError) as exc:
        raise AIProviderInvalidResponseError(f"{provider} did not return valid JSON: {exc}") from exc
    ...
    lat_min, lat_max = KENYA_LAT_BOUNDS
    lng_min, lng_max = KENYA_LNG_BOUNDS
    if not (lat_min <= lat <= lat_max and lng_min <= lng <= lng_max):
        raise AIProviderInvalidResponseError(
            f"{provider} returned coordinates ({lat}, {lng}) well outside Kenya - discarding rather than "
            "trusting an implausible geocode."
        )

    return AIGeocodeResult(lat=lat, lng=lng, reasoning=reasoning, provider=provider, model=model)
