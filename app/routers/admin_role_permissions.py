...
def update_role_permissions(
    ...
    # Previously every role was forced to always keep Overview, since the
    # frontend's login/redirect logic assumed "/admin" was always a safe
    # landing page. That's no longer true - AdminLayout.tsx and
    # AdminLogin.tsx now compute each role's own actual first allowed page
    # instead of hardcoding "/admin" (see getLandingPath() in
    # src/lib/roleAccess.ts), so Overview can be safely turned off for a
    # role like any other menu entry - nothing to force here anymore.
    allowed = list(dict.fromkeys(payload.allowed_menus))  # de-duplicate while preserving order
